...
(function() {
  ...
  RichMarkerBuilder = (function(_super) {
    ...
    RichMarkerBuilder.prototype.create_infowindow = function() {
      ...
      boxText.setAttribute("class", 'infoboxStyle');
      boxText.innerHTML = this.args.infowindow + "<br />" + this.args.address;
  ...
  this.buildMap = function(markers) {
    ...
    return handler.buildMap({
      ...
      return $(".mapEventName").on("click", function() {
        var i, marker;
        i = $(this).attr("id");
        marker = markers[i];
        return google.maps.event.trigger(marker.getServiceObject(), "click");
      });
    });
  };

  $(document).ready(function() {
    return $("#new_event").on("ajax:success", function(e, data, status, xhr) {
      if (status === "success") {
        $('.alert-success').text('Successfully added event').addClass('show');
        return $('#new_event')[0].reset();
      }
    }).bind("ajax:error", function(e, xhr, status, error) {
      return $("#new_event").append("<p>ERROR</p>");
    });
  });
